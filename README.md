<h1 align="left"><img src="docs/spacer.webp" width="10" alt=""><img src="docs/wordmark.webp" alt="ComfyUI-PromptChain" width="460"></h1>
...
A prompt-authoring and image-iteration suite for [ComfyUI](https://github.com/comfyanonymous/ComfyUI). PromptChain turns the raw prompt box into a model-aware, structured workflow — and carries that structure through posing, generating, viewing, editing, and upscaling.

## Prompt authoring

- **Prompt Chain node** — a real code editor on the node: syntax highlighting, comments, multi-line editing, and a collapsible output panel showing the compiled positive/negative prompt, a live server console, and this node's gallery.
- **Chain modes** — link multiple Prompt Chain nodes and choose how their text merges: **Combine** all, **Randomize** one at random, **Switch** to a chosen branch, or **Iterate** through options across queued runs — shown live in the toolbar.
...
- **Props** — drop in matte primitives and a large categorized library (furniture, weapons, vehicles, tools, environment), spawn pre-arranged prop sets, wear calibrated shoes, or **import your own `.glb`/`.obj`** — and even fit a skeleton inside an import to make it posable.
...
- **Self-installing features** — Style Reference, Upscaler, Face Detailer, ControlNet, PuLID, SeedVR2, and AI Selection install their own nodes and model files on first use, with live progress and a restart — no Manager round-trip. A health dashboard in **Settings → PromptChain → Install** shows status, sizes, and updates.
...
Add the **Prompt Chain** node from the node menu — onboarding takes it from there.
