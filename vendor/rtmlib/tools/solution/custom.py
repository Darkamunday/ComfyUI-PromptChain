...
from ComfyUI_PromptChain.vendor.rtmlib import Custom, draw_skeleton
...
def _rtmlib_root():
    # Vendored: resolve the package root by its own relative path (…vendor.rtmlib),
    # never the top-level name 'rtmlib', so a pip-installed rtmlib can't shadow us
    # and import never fails when this slice lives under a renamed parent package.
    return importlib.import_module(__name__.rsplit('.tools.', 1)[0])
    ...
    def __init__(self,
        ...
        if det_class is not None:
            try:
                ...
                det_class = getattr(_rtmlib_root(), det_class)
        ...
        if pose_class is not None:
            try:
                pose_class = getattr(_rtmlib_root(), pose_class)
